                        ...
                        <div class="row mg-t-20">
                            ...
                            <div class="col-md">
                                ...
                                <p class="invoice-info-row"><span>رقم الفاتورة</span>
                                    <span>{{ $invoice->details?->invoice_number }}</span></p>
                                <p class="invoice-info-row"><span>تاريخ الاصدار</span>
                                    <span>{{ $invoice->details?->invoice_date }}</span></p>
                                <p class="invoice-info-row"><span>تاريخ الاستحقاق</span>
                                    <span>{{ $invoice->details?->due_date }}</span></p>
                            ...
                            <table class="table table-invoice border text-md-nowrap mb-0">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="tx-center">{{ number_format($invoice->details?->amount_collection, 2) }}</td>
                                        <td class="tx-right">{{ number_format($invoice->details?->amount_commission, 2) }}</td>
                                        @php
                                        $total = $invoice->details?->amount_collection + $invoice->details?->amount_commission;
                                        @endphp
                                        <td class="tx-right">
                                    ...
                                    <tr>
                                        <td class="tx-right">نسبة الضريبة ({{ $invoice->details?->rate_vat }})</td>
                                        <td class="tx-right" colspan="2">287.50</td>
                                    </tr>
                                    <tr>
                                        <td class="tx-right">قيمة الخصم</td>
                                        <td class="tx-right" colspan="2"> {{ number_format($invoice->details?->discount, 2) }}</td>

                                    </tr>
                                    <tr>
                                        <td class="tx-right tx-uppercase tx-bold tx-inverse">الاجمالي شامل الضريبة</td>
                                        <td class="tx-right" colspan="2">
                                            <h4 class="tx-primary tx-bold">{{ number_format($invoice->details?->total, 2) }}</h4>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
